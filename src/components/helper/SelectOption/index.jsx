...
export default function SelectOption(
  {options, onChange, value,label, leftSibling=null,
    ...
    <div style={{position:"relative", width:"100%"}} onClick={() => toggle(!show)} >
        <div className={`${containerClass || 'w-full bg-white cursor-pointer flex justify-between border rounded-md text-sm tablet:text-base py-2.5 px-3 gap-x-2 items-center  mt-[4px] '} ${isInput && value && !options.includes(value)?' border-red-100 ':''}`}  
            >
        ...
        {show && (
            <ul className={'absolute bg-white w-full rounded overflow-auto min-w-fit'} style={{
