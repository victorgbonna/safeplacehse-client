...
export default function SelectMultiple(
  {options, onChange, values,label, leftSibling=null,valueProp,
    ...
    <div style={{position:"relative", width:"100%"}} onClick={!isInput?() => toggle(!show):()=>null} >
        ...
        {!isInput?<div 
        className={`${containerClass || 'w-full bg-white cursor-pointer flex justify-between border rounded-md text-sm tablet:text-base py-2.5 px-3 gap-x-2 items-center  mt-[4px] '}`}  
            >
        ...
        :<input className={`${containerClass || 'w-full bg-white  border rounded-md text-sm tablet:text-base py-2.5 px-3 gap-x-2 items-center  mt-[4px] '}`}
            value={text} onChange={(e)=>setText(e.target.value)}
        /> } 
        {show && (
            <ul className={' h-fit absolute bg-white w-full rounded overflow-auto min-w-fit '+extraClass} style={{
